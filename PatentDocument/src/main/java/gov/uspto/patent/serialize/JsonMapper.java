...
public class JsonMapper implements DocumentBuilder<Patent> {
	...
	public JsonObject buildJson(Patent patent) {
		...
		builder.add("documentId", patent.getDocumentId() != null ? patent.getDocumentId().toText() : ""); // Patent ID
																											// or Public
																											// Application
																											// ID.
	 ...
	 * JsonReader reader = Json.createReader(new StringReader(jsonStr)); JsonObject
	 * jsonObj = reader.readObject(); String type = jsonObj.getString("type");
	 * String docId = jsonObj.getString("documentId");
	 * 
	 * if (type.equalsIgnoreCase("USPAT")) { PatentGranted patent = new
	 * PatentGranted(DocumentId.fromText(docId)); return readPatent(jsonObj,
	 * patent); } else if (type.equalsIgnoreCase("US-PGPUB")) { PatentApplication
	 * patent = new PatentApplication(DocumentId.fromText(docId)); return
	 * readApplication(jsonObj, patent); } return null; }
	 */
