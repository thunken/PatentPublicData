 ...
 * 
 * --source google --type application --limit 1 --source reedtech --type
 * application --limit 1 --source cpc --limit 1
 * 
...
public class Download {
	 ...
	 * 
	 * @param skip
	 *            - number of files to skip over.
	 * @param limit
	 *            - max amount of files to download, limit of 0 is unlimited.
	 * @return
	 * @throws IOException
	 */
	...
	public static void main(String... args) throws IOException, JAXBException {
		...
		OptionParser parser = new OptionParser() {
			{
				accepts("source").withRequiredArg().ofType(String.class)
						.describedAs("source provider: [google, reedtech, cpc]").required();
				accepts("type").withOptionalArg().ofType(String.class)
						.describedAs("data type: [patent_grant, patent_application, ]");
				...
				accepts("available").withOptionalArg().ofType(String.class).describedAs("Show Available Sources.");
			}
		};
		...
		if (options.has("available")) {
			...
			for (Source source : sourceList) {
				System.out.println("--source=" + source.getName().toLowerCase() + " " + "--type="
						+ source.getDocType().toLowerCase());
			}
			System.exit(0);
		}
