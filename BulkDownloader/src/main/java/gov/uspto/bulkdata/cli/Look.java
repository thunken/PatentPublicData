 ...
 *
 * View contents of listed fields --source="download/ipa150101.zip" --limit 5
 * --skip 5 --fields=id,title,family
 * 
 * Dump a single Patent XML Document by location in zipfile; the 3rd document:
 * --source="download/ipa150305.zip" --num=3 --fields=xml
 * --out=download/patent.xml
 * 
 * Dump a single Patent XML Document by ID (note it may be slow as it parse each
 * document to check its id): --source="download/ipa150305.zip" --id=3
 * --fields=xml --out=download/patent.xml
 * 
	...
	private void show(Patent patent, String[] fields, Writer writer) throws IOException {
		for (String field : fields) {
			...
			case "description":
				...
				// patent.getDescription().getPlainText());
				break;
			case "citations":
			...
			case "family":
				...
				for (DocumentId docId : patent.getRelationIds()) {
					// System.out.println("\t" + docId.getDocIdType().name() + " : " +
					// docId.toText() );
		...
		OptionParser parser = new OptionParser() {
			{
				...
				accepts("fields").withOptionalArg().ofType(String.class).describedAs(
